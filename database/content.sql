...
CREATE TABLE `content`.`image_file_infos` (
  ...
  `storage_file` INTEGER UNSIGNED COMMENT 'storage.files.id FK',
...
ALTER TABLE `content`.`image_file_infos` ADD FOREIGN KEY (`storage_file`) REFERENCES `storage`.`files` (`id`);
...
ALTER TABLE `content`.`pdf_file_infos` ADD FOREIGN KEY (`storage_file`) REFERENCES `storage`.`files` (`id`);
